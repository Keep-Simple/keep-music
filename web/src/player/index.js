...
export default class ReactJkMusicPlayer extends PureComponent {
    ...
    state = {
        ...
        playIndex:
            this.props.playIndex ||
            this.props.defaultPlayIndex ||
            DEFAULT_PLAY_INDEX,
        canPlay: false,
    ...
    get isAudioCanPlay() {
        ...
        const {
            isInitAutoPlay,
            isAutoPlayWhenUserClicked,
            canPlay,
        } = this.state
        return (
            canPlay && (isInitAutoPlay || autoPlay || isAutoPlayWhenUserClicked)
        )
    }
    ...
    renderAudioTitle = () => {
        ...
        if (this.props.renderAudioTitle) {
            return this.props.renderAudioTitle(
                this.getBaseAudioInfo(),
                isMobile
            )
        }
    ...
    openAudioListsPanel = () => {
        ...
        this.props.onAudioListsPanelChange &&
            this.props.onAudioListsPanelChange(
                !this.state.audioListsPanelVisible
            )
    }
    ...
    onAudioPause = () => {
        ...
        this.props.onAudioPause &&
            this.props.onAudioPause(this.getBaseAudioInfo())
    ...
    checkCurrentPlayingAudioIsInUpdatedAudioLists = (
    ...
    getPlayId = (audioLists = this.state.audioLists) => {
        ...
        const playId =
            this.state.playId ||
            (audioLists[playIndex] && audioLists[playIndex].id)
        return playId
    }
    ...
    updateMediaSessionPositionState = () => {
        if ('setPositionState' in navigator.mediaSession) {
            try {
                ...
                // eslint-disable-next-line no-console
                console.error(
                    'Update media session position state failed: ',
                    error
                )
            }
        }
